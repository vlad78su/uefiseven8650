  ...
  )
{
  ...
  *(UINT16 *)BufferPtr = 0x00F1;   // Номер режима
  BufferPtr += 2;
  *(UINT16 *)BufferPtr = 0xFFFF;
  ...
  // Хак для бутскрина: заставляем Windows думать, что экран 1024x768
  VbeModeInfo->Width                    = 1024;   
  VbeModeInfo->Height                   = 768;    
  VbeModeInfo->CharCellWidth            = 8;      
  VbeModeInfo->CharCellHeight           = 16;     

  // Центрируем: высчитываем сдвиг фреймбуфера. 
  // Экран инициализирован в 1366, картинка будет 1024. Смещение составит ровно 171 пиксель.
