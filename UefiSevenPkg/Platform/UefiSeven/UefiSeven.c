  ...
  )
{
  ...
  //
  // Force UEFI GOP to native panel resolution (1366x768)
  //
  ...
  //
// 1. Очищаем всю область VGA ROM (128 КБ)
  ZeroMem ((VOID *)VGA_ROM_ADDRESS, VGA_ROM_SIZE);

  // 2. Копируем твой родной VBIOS в самое начало области 0xC0000
  CopyMem ((VOID *)VGA_ROM_ADDRESS, AMD_VBIOS, sizeof (AMD_VBIOS));

  // 3. Высчитываем адрес в памяти СРАЗУ за твоим VBIOS для VBE структур
  // Выравниваем адрес, например, на 2 КБ вперед, чтобы ничего не наложилось
  EFI_PHYSICAL_ADDRESS VesaStartAddress = VGA_ROM_ADDRESS + ((sizeof(AMD_VBIOS) + 2047) & ~2047);
  EFI_PHYSICAL_ADDRESS VesaEndAddress;

  // 4. Вызываем родной генератор VBE от UefiSeven на этот свободный адрес
  Status = ShimVesaInformation (VesaStartAddress, &VesaEndAddress);
  if (EFI_ERROR (Status)) {
    PrintDebug (L"Failed to generate VBE structures after AMD VBIOS: %r\n", Status);
  } else {
    PrintDebug (L"VBE structures generated successfully at offset %x\n", VesaStartAddress);
  }

  // 5. Точка входа Int10h. 
  // ВНИМАНИЕ: Если мы хотим, чтобы работал безопасный режим, прерывание Int10h 
  // должно обрабатываться эмулятором UefiSeven (Int10hHandler), а не прыгать в код VBIOS!
  // Поэтому возвращаем стандартную логику UefiSeven для вектора прерывания:
  
  extern VOID *Int10hHandlerEntryPoint; // Проверь, как она объявлена в Int10hHandler.h
  NewInt10hHandlerEntry.Segment = (UINT16)((UINT32)&Int10hHandlerEntryPoint >> 4);
  NewInt10hHandlerEntry.Offset  = (UINT16)((UINT32)&Int10hHandlerEntryPoint & 0xF);

  //
  // Int10h Address
  //
  NewInt10hHandlerEntry.Segment = (UINT16)((UINT32)VGA_ROM_ADDRESS >> 4); // Получится сегмент 0xC000
  NewInt10hHandlerEntry.Offset  = 0x0000; 
